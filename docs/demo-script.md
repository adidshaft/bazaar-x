...
"We chose X Layer because the hackathon rewards live onchain execution, not mock logic. Bazaar X uses X Layer for real settlements, treasury flows, and governance state, while Onchain OS gives us a practical agent-wallet and transaction path."
...
"Bazaar X turns agents into economic actors. Covenant Skill makes the policy engine reusable for any other agent economy. This is the piece that can extend beyond the hackathon."
